...
function setupDevServer(config) {
  config.setupMiddlewares = (middlewares, devServer) => {
    ...
    const isAllowedOrigin = (origin) => {
      ...
      // Allow only localhost and 127.0.0.1 (and App Engine appspot) by default
    ...
    devServer.app.post("/edit-file", (req, res) => {
      ...
      try {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          traverse(ast, {
            JSXOpeningElement: (path) => {
              ...
              changesAtLine.forEach((change) => {
                ...
                console.log(
                  `[backend] Processing change type: ${
                    change.type || "legacy"
                ...
                if (
                  ...
                  const reason = `Change must have valid type ('className', 'textContent', or 'content'). Received type: ${
          ...
          // Note: automatic git commits were removed for privacy/safety.
